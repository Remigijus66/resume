...
const Day = ({ day }) => {
  ...
  return (
    <>
      ...
      {(hourly ? ungrouped : grouped)
        .filter((interval) => { return isInFuture(interval.time) || notToday(day.date) })
        .map((interval, i) => {
          return <div className='wheather-line  thin-divider' style={{ fontSize: '14px' }} key={i}>
            <span className="first"> {interval.time} </span>
            <span className="second" ><img style={{ width: '20px' }} src={interval.condition.icon} alt="" /></span>
